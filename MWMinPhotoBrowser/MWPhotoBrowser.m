...
//    }
    ...
    if (self.toolbarItems && self.toolbarItems.count != 0) {
        hideToolbar = NO;
        [_toolbar setItems:self.toolbarItems];
    }
...
- (void)layoutVisiblePages {
	...
	// Toolbar
	_toolbar.frame = [self frameForToolbarAtOrientation:self.view.window.windowScene.interfaceOrientation];
...
- (CGRect)frameForSelectedButton:(UIButton *)selectedButton atIndex:(NSUInteger)index {
    ...
    CGFloat statusBarOffset = self.view.window.windowScene.statusBarManager.statusBarFrame.size.height;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < __IPHONE_7_0
...
- (void)setControlsHidden:(BOOL)hidden animated:(BOOL)animated permanent:(BOOL)permanent {
    ...
    if (slideAndFade && [self areControlsHidden] && !hidden && animated) {
        ...
        // Toolbar
        _toolbar.frame = CGRectOffset([self frameForToolbarAtOrientation:self.view.window.windowScene.interfaceOrientation], 0, animatonOffset);
...
- (void)mailComposeController:(MFMailComposeViewController*)controller didFinishWithResult:(MFMailComposeResult)result error:(NSError*)error {
    if (result == MFMailComposeResultFailed) {
        UIAlertController * alert = [UIAlertController
                                     alertControllerWithTitle:NSLocalizedString(@"Email", nil)
